	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPost {
			...
			if user == nil {
				...
				target, err := models.FindUsernameOrEmail(username)
				if err != nil {
			...
			if token.Token != "" {
				...
				if err := user.Save(); err != nil {
					...
					// Log this event to the change log.
					models.LogEvent(user, nil, models.ChangeLogLifecycle, "users", user.ID, "Reset their forgotten password via e-mail.")
